...
def get_weighted_choice(present_class_indices):
    ...
    for row in students_data:
        ...
        try:
            ...
            # Пропускаємо, якщо не вдалося декодувати face_encoding
            continue
        ...
        if is_immune:
            continue  # Пропустити, якщо має імунітет
    ...
    # Знаходимо мінімальну кількість виходів, щоб уникнути ділення на нуль або великих чисел
    min_times = min(s['times'] for s in available_students)

    weights = []
    # Параметр випадковості: 0.15 означає, що 15% впливу на вибір — це чиста вдача
    randomness_factor = 0.15

    for student in available_students:
        relative_times = student['times'] - min_times

        # Основна математична вага (алгоритмічна частина)
        base_weight = 100.0 / ((relative_times + 1) ** 2)

        # Додаємо "шум". Навіть якщо base_weight дуже малий,
        # randomness дає учню шанс не розслаблятися.
    ...
    if total_weight == 0:
        ...
        # Тепер використовуємо наші змішані ваги (математика + рандом)
